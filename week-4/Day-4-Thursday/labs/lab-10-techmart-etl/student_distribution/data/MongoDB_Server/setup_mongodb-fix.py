...
FIXED VERSION: Handles duplicate key errors gracefully
"""
...
def setup_mongodb():
    ...
    print("🍃 TechMart MongoDB Database Setup (FIXED VERSION)")
    print("=" * 50)
    ...
    try:
        ...
        if isinstance(customer_data, list):
            # Use ordered=False to continue inserting even if duplicates are found
            try:
                ...
                successful_inserts = (
                    len(e.details["writeErrors"]) if "writeErrors" in e.details else 0
                )
                total_attempted = len(customer_data)
                actual_inserted = total_attempted - successful_inserts
                print(
                    f"✅ Imported {actual_inserted} customer profiles (skipped {successful_inserts} duplicates)"
                )
        else:
            try:
                ...
                print("✅ Customer profile already exists (skipped duplicate)")
