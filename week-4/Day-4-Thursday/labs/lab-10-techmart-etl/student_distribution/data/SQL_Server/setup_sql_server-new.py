...
def create_database_and_tables():
    ...
    connection_string = f"DRIVER={{ODBC Driver 18 for SQL Server}};SERVER={SERVER};DATABASE=master;UID={USERNAME};PWD={PASSWORD};TrustServerCertificate=yes;"
...
def import_csv_data(conn, cursor):
    ...
    try:
        ...
        for _, row in categories_df.iterrows():
            cursor.execute(
                """
                INSERT INTO ticket_categories (category_id, category_name, category_description, priority_level)
                VALUES (?, ?, ?, ?)
            """,
                row["category_id"],
                row["category_name"],
        ...
        for _, row in customers_df.iterrows():
            cursor.execute(
                """
                INSERT INTO customers (customer_id, customer_uuid, first_name, last_name, email, phone, registration_date, customer_status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                row["customer_id"],
                row["customer_uuid"],
