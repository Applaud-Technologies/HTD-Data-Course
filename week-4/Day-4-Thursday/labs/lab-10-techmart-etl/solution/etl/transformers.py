...
def clean_customer_data(customers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for idx, customer in enumerate(customers):
        try:
            ...
            cleaned_customer["email"] = email

            # Handle phone
            phone = customer.get("phone", "")
            if phone is not None:
                phone = str(phone).strip()
            cleaned_customer["phone"] = phone
        ...
        try:
            ...
            if quantity is not None:
                try:
                    ...
                    if quantity <= 0:
                        logger.warning(
                            f"Invalid quantity {quantity} for transaction {transaction_id}, setting to 1"
                        )
                        quantity = 1.0
                except (ValueError, TypeError):
                    logger.warning(
                        f"Invalid quantity for transaction {transaction_id}, setting to 1"
                    )
                    quantity = 1.0
            else:
                quantity = 1.0
            ...
            if unit_price is not None:
                try:
                    ...
                    if unit_price < 0:
                        logger.warning(
                            f"Negative price {unit_price} for transaction {transaction_id}, setting to 0"
                        )
                        unit_price = 0.0
                except (ValueError, TypeError):
                    unit_price = 0.0
                ...
                unit_price = 0.0
            cleaned_sale["unit_price"] = unit_price

            # Calculate totals
            cleaned_sale["item_total"] = quantity * unit_price
            cleaned_sale["final_total"] = cleaned_sale["item_total"]
            ...
            if transaction_date:
                try:
                    ...
                    if parsed_date > datetime.now():
                        logger.warning(
                            f"Future date {parsed_date} for transaction {transaction_id}, using today"
                        )
                        parsed_date = datetime.now()
                    cleaned_sale["transaction_date"] = parsed_date.strftime("%Y-%m-%d")
                except:
                    cleaned_sale["transaction_date"] = datetime.now().strftime(
                        "%Y-%m-%d"
                    )
            else:
...
def apply_scd_type1_logic(
    ...
    SCD Type 1 overwrites existing records with new values when changes are detected.
    ...
    try:
        ...
        for dim_name, new_records in dimension_data.items():
            ...
            # Process new records for SCD Type 1
        ...
        scd_operations["total_inserts"] = (
            scd_operations["customer_inserts"]
            + scd_operations["product_inserts"]
            + scd_operations["store_inserts"]
            + scd_operations["category_inserts"]
        )

        scd_operations["total_updates"] = (
            scd_operations["customer_updates"]
            + scd_operations["product_updates"]
            + scd_operations["store_updates"]
            + scd_operations["category_updates"]
        )

        logger.info("SCD Type 1 logic applied successfully")
        logger.info(
            f"SCD Operations: {scd_operations['total_inserts']} inserts, {scd_operations['total_updates']} updates"
        )
