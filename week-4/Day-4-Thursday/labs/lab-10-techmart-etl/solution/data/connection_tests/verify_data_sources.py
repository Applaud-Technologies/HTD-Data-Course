...
def check_csv_files():
    ...
    csv_file = "data/Source_CSV/daily_sales_transactions.csv"
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
        print(f"   ✅ daily_sales_transactions.csv - {len(df)} records")
        print(f"      Columns: {list(df.columns)}")
        return True
    else:
        print(f"   ❌ daily_sales_transactions.csv - File missing")
        return False
...
def check_json_files():
    ...
    json_file = "data/Source_JSON/product_catalog.json"
    if os.path.exists(json_file):
        with open(json_file, "r") as f:
            data = json.load(f)

        if isinstance(data, list):
            print(f"   ✅ product_catalog.json - {len(data)} records")
            if data:
                print(f"      Sample keys: {list(data[0].keys())}")
        elif isinstance(data, dict):
            if "products" in data:
                products = data["products"]
                print(f"   ✅ product_catalog.json - {len(products)} records")
                if products:
                    print(f"      Sample keys: {list(products[0].keys())}")
            else:
                print(
                    f"   ⚠️ product_catalog.json - Unknown structure: {list(data.keys())}"
                )
        return True
    else:
        print(f"   ❌ product_catalog.json - File missing")
        return False
